...
@if($order->order_by === "Staff")
  ...
  <div class="checkout-page">
        ...
        <thead>
          ...
          <tbody id="checkout-render">
            @foreach($data['checkout_summary']['products'] as $key=>$row)
                <tr>
                  <td>
                    <a id="{{$row['product']['id']}}" target="_blank" href="{{route('admin.orders.show', encrypt($order->id))}}">
    ...
    <div class="d-flex justify-content-center action-buttons-group">
      @if($order->status === "New")
        <a onclick="return confirm('Are you sure to Move to In Progress')" href="{{route('admin.order.actions', [encrypt($order->id), encrypt('In Progress')])}}" class="btn btn-secondary btn-sm mr-1" title="Move to In Progress">In Progress</a>
        <a onclick="return confirm('Are you sure to Move to Out For Delivery')" href="{{route('admin.order.actions', [encrypt($order->id), encrypt('Out For Delivery')])}}" class="btn btn-info btn-sm mr-1" title="Move to Out For Delivery">Out For Delivery</a>
        <a onclick="return confirm('Are you sure to Move to Completed')" href="{{route('admin.order.actions', [encrypt($order->id), encrypt('Completed')])}}" class="btn btn-success btn-sm mr-1" title="Move to Completed">Completed</a>
        <a onclick="return confirm('Are you sure to Cancel')" href="{{route('admin.order.actions', [encrypt($order->id), encrypt('Cancelled')])}}" class="btn btn-danger btn-sm" title="Cancel this order">Cancel</a>
      @endif
