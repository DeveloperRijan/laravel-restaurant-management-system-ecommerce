  ...
  <div class="row">
    <div class="col-lg-12 col-md-12">
        <div class="text-center" style="margin-top: 20px">
          ...
          @if(\Session::has('error_msg_order_page'))
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
              {{ session()->get('error_msg_order_page') }}
            </div>
          @endif
        </div>
      ...
      @elseif(\Request::get('step') == 'secure_payment')
      <h4 class="checkout-heading">Checkout - (Payment)</h4>
      @else
      <h4 class="checkout-heading">Checkout - (Item Detail)</h4>
      @endif
    </div>

    @if(\Request::get('step') == 'shipping_address')
      @include("frontendViews.staff.partials.checkout.addresses")

    @elseif(\Request::get('step') == 'secure_payment')

        <?php
          //get session checkout data
          $data = \Session::get("staff_checkout_whole_data");
          $staffCreditBalance = \App\Models\StaffCredit::where('user_id', \Auth::user()->id)->first();
        ?>

        @if(isset($data['checkout_summary']['summary']['grand_total']))
        <div class="payment-hints text-center">
          <b>Your Bill : {{env("CURRENCY_SYMBOL").$data['checkout_summary']['summary']['grand_total']}}</b>
        </div>
          @include("frontendViews.staff.partials.checkout.payment_gateway")
        @endif

    @else
      @include("frontendViews.staff.partials.checkout.items_review")
    @endif
...
@push("scripts")
<script type="text/javascript" src="{{$publicAssetsPathStart}}frontend/js/staff_checkout_control.js"></script>
@endpush