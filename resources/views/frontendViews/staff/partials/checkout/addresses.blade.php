<?php
  $addresses = \App\Models\StaffAddress::where('user_id', Auth::user()->id)->orderBy("created_at", "DESC")->get();
?>
  ...
  <div>
    ...
    <div>
      <div class="row">
        ...
        <div class="col-lg-6 col-md-12">
          <form id="addressUpdateForm" action="{{route('staff.address.update')}}" method="POST">
              @csrf
              <div id="address-preview-form-wrapper" class="d-none"
                  ...
                  <div>
                    @include("frontendViews.staff.partials.checkout.address_preview")
                  </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="addNewAddressForm" method="POST" action="{{route('staff.address.add')}}">
          @csrf
...
<script type="text/javascript">
  $("select[name='shipping_address']").on("change", function(){
    ...
    //get address details
    $.ajax({
        url: "/staff/my-addresses?get_address_details_by_id="+$(this).val(),
        method: "GET",
        dataType: 'HTML',
        cache: false,
        beforeSend:function(){
        ...
        error: function (jqXHR, textStatus, errorThrown) {
          $("#form__processing__gif").hide()
          $("#address-preview-form-wrapper").addClass('d-none')
              alert("Address Not Found / Something wrong")
          },
          complete:function(){
            $("#form__processing__gif").hide()
          }
  ...
  function returnToPaymentGateway(e){
    ...
    window.location.href = "{{route('staff.order.page')}}?step=payment&address="+addressID
  }
