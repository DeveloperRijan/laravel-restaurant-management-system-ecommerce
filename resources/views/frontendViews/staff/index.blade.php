...
@section("content")
    ...
    <section id="popular_foods">
        ...
        @if($staffContentRow1 !== NULL && count($staffContentRow1) > 0)
            @include("frontendViews.staff.partials.item-list", ['products'=>$staffContentRow1])
        @endif
        </div>
    </section>
    <!-- foods section ends -->
...
<h1 class="home_section_headerTxt">Family Orders </h1>
            @if($staffContentRow2 !== NULL && count($staffContentRow2) > 0)
                @include("frontendViews.staff.partials.item-list", ['products'=>$staffContentRow2])
            @endif
